...
class ChiSurvey
{
    ...
    public function generate_question_html($question_obj) {
        ...
        $out =  "<div class='chi-enquete-question-block'><div class='chi-enquete-question'>".
            "<span class='chi-enquete-question-number'>{$question_number}</span>".
            "<span class='chi-enquete-actual-question'> {$bare_question}</span></div>" .
            "<fieldset><div class='chi-enquete-answer-line'>";
        ...
        foreach ($answer_array as $answer) {
            ...
            $out.= "<div class='chi-enquete-radio-and-label'> <input type='radio' class='chi-enquete-radio' name='$answer_name' value='$answer_value' id='$answer_id' />" .
                "<label for='$answer_id' class='chi-enquete-radio-label'>$answer_text</label></div>";
        }
    ...
    protected function get_unique_code()
    {
        ...
        while ($counter) {
            ...
            $find_code = $wpdb->get_results(
                "select id  from {$wpdb->base_prefix}chi_enquete_survey WHERE anon_key = '%s'", $uid
            );
            if (empty($find_code)) {
                return $uid;
            }
...
Energie	='VitaCheck 2018'!C25 = final_energy
        ...
        global $wpdb;
        $questions_table_name = $wpdb->prefix . 'chi_enquete_questions';
        $answers_table_name = $wpdb->prefix . 'chi_enquete_answers';
        $survey_table = $wpdb->prefix . 'chi_enquete_survey';
